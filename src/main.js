...
const hospitalSuggestions = new Set()
const vehicleSuggestions = new Set()
const HOSPITAL_SUGGESTIONS_KEY = 'mci_hospital_suggestions'
const VEHICLE_SUGGESTIONS_KEY = 'mci_vehicle_suggestions'
...
function populateFormFromEntry(entry) {
  const serialEl = $('serialNo')
  if (serialEl) serialEl.value = entry.serial_no != null ? String(entry.serial_no) : ''
  const nameEl = $('name')
...
async function exportIncidentEntriesToExcel() {
  ...
  const rows = data.map((entry, i) => ({
    연번: entry.serial_no != null ? entry.serial_no : i + 1,
...
function parseSerialNo(rawValue) {
...
function clampSerialNo(target, totalCount) {
  if (!Number.isFinite(target)) return null
  if (totalCount <= 0) return 1
  return Math.max(1, Math.min(target, totalCount))
}

async function reorderIncidentSerials(movingEntryId, desiredSerialNo) {
  if (!supabase || !currentIncidentId || !movingEntryId || !Number.isFinite(desiredSerialNo)) return null
  ...
  const currentIndex = data.findIndex((r) => r.id === movingEntryId)
  if (currentIndex < 0) return null

  const targetIndex = clampSerialNo(desiredSerialNo, data.length) - 1
  if (targetIndex !== currentIndex) {
    const [moving] = data.splice(currentIndex, 1)
    data.splice(targetIndex, 0, moving)
  }

  const updates = data.map((row, idx) =>
    supabase.from('mci_casualty_entries').update({ serial_no: idx + 1 }).eq('id', row.id)
  )
...
async function normalizeIncidentSerials() {
  if (!supabase || !currentIncidentId) return null
  const { data, error } = await supabase
    .from('mci_casualty_entries')
    .select('id')
    .eq('incident_id', currentIncidentId)
...
function updateDatalist(elId, valuesSet) {
  const el = $(elId)
  if (!el) return
  const sorted = [...valuesSet].sort((a, b) => a.localeCompare(b, 'ko'))
  el.innerHTML = sorted.map((v) => `<option value="${escapeHtml(v)}"></option>`).join('')
}

function loadSuggestionsFromStorage() {
  try {
    const hRaw = localStorage.getItem(HOSPITAL_SUGGESTIONS_KEY)
    if (hRaw) {
      JSON.parse(hRaw).forEach((v) => {
        const n = normalizeSuggestionValue(v)
        if (n) hospitalSuggestions.add(n)
      })
    }
    const vRaw = localStorage.getItem(VEHICLE_SUGGESTIONS_KEY)
    if (vRaw) {
      JSON.parse(vRaw).forEach((v) => {
        const n = normalizeSuggestionValue(v)
        if (n) vehicleSuggestions.add(n)
      })
    }
  } catch (_) {
    // 저장소 데이터가 깨졌으면 무시
  }
  updateDatalist('hospitalOptions', hospitalSuggestions)
  updateDatalist('vehicleOptions', vehicleSuggestions)
}

function saveSuggestionsToStorage() {
  try {
    localStorage.setItem(HOSPITAL_SUGGESTIONS_KEY, JSON.stringify([...hospitalSuggestions]))
    localStorage.setItem(VEHICLE_SUGGESTIONS_KEY, JSON.stringify([...vehicleSuggestions]))
  } catch (_) {
    // 저장소 용량/권한 오류는 무시
  }
}

function addSuggestionAndRefresh(type, value) {
  const normalized = normalizeSuggestionValue(value)
  if (!normalized) return
  if (type === 'hospital') {
    hospitalSuggestions.add(normalized)
    updateDatalist('hospitalOptions', hospitalSuggestions)
  } else if (type === 'vehicle') {
    vehicleSuggestions.add(normalized)
    updateDatalist('vehicleOptions', vehicleSuggestions)
  }
...
function buildRowHtml(entry, indexFromNewest) {
  ...
  const serialNo = entry.serial_no != null ? entry.serial_no : indexFromNewest
  return `
    <td data-label="연번">${escapeHtml(serialNo)}</td>
...
function buildMobileCardHtml(entry, indexFromNewest) {
  ...
  return `<div class="entry-card data-row-clickable" data-entry-id="${eid}">
    <div class="entry-card-line1"><span class="ec-no">${serialNo}</span><span>${name || '—'}</span><span> · </span><span>${genDisp}</span><span> · </span><span>${ageDisp}</span><span> · </span><span class="ec-symptom">${symptom}</span></div>
...
async function loadEntries() {
  ...
  data.forEach((row, i) => {
    ...
    if (row.transfer_status === '이송') {
      addSuggestionAndRefresh('hospital', row.destination_hospital || '')
      addSuggestionAndRefresh('vehicle', row.transfer_vehicle || '')
    }
  })
...
async function saveEntry() {
  ...
  const payload = {
    serial_no: parseSerialNo(val('serialNo')),
  ...
  const hintFn = (msg) => {
    ...
    if (/serial_no/i.test(m)) {
      h += ' supabase/mci_add_serial_no.sql 실행 여부를 확인하세요.'
    }
    return h
  }
  ...
  if (isEdit) {
    ...
    const res = await supabase.from('mci_casualty_entries').insert(payload).select('id').single()
    error = res.error
    savedEntryId = res.data?.id || null
  }
  ...
  if (payload.transfer_status === '이송') {
    addSuggestionAndRefresh('hospital', payload.destination_hospital || '')
    addSuggestionAndRefresh('vehicle', payload.transfer_vehicle || '')
  }

  if (payload.serial_no != null && savedEntryId) {
    const reorderError = await reorderIncidentSerials(savedEntryId, payload.serial_no)
    if (reorderError) {
      setDbStatus(`연번 재정렬 실패: ${friendlySupabaseMessage(reorderError)}${hintFn(reorderError.message)}`, true)
      if (btn) btn.disabled = false
      return
    }
  }
...
async function deleteEditingEntry() {
  ...
  if (!editingEntryId) {
    setDbStatus('삭제할 항목을 먼저 선택하세요.', true)
    return
  }
  if (!confirm('선택한 사상자 기록을 삭제할까요?')) return

  const btnDelete = $('btnDeleteEntry')
  if (btnDelete) btnDelete.disabled = true
  setDbStatus('삭제 중…', false)

  const { error } = await supabase.from('mci_casualty_entries').delete().eq('id', editingEntryId)
  if (error) {
    setDbStatus(`삭제 실패: ${friendlySupabaseMessage(error)}`, true)
    if (btnDelete) btnDelete.disabled = false
    return
  }

  const reorderError = await normalizeIncidentSerials()
  if (reorderError) {
    setDbStatus(`연번 재정렬 실패: ${friendlySupabaseMessage(reorderError)}`, true)
    if (btnDelete) btnDelete.disabled = false
    return
  }
...
function wireRecordForm() {
  ...
  $('btnDeleteEntry')?.addEventListener('click', deleteEditingEntry)
